...
filename = './video/' + now.strftime('%Y%m%d_%H_%M') + '.mp4'
now_l = now.strftime('%Y%m%d_%H_%M')

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='tf-pose-estimation run')
    parser.add_argument('--video', type=str, default='./movie/test.mp4')
    ...
    writer = cv2.VideoWriter(filename, fourcc, count, (w, h))
...
#        cv2.imwrite(args.datas + args.dataname +'_'+ args.movie[-len(args.movie)+8:-4] + '_'  + str(i) +  '_data.jpg', frame)
    ...
    dfs.to_csv('csv/{}.csv'.format(now_l),encoding='utf-8')
    writer.release()
