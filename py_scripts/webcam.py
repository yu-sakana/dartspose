...
filename = './video/{}'.format(now.strftime('%Y_%m%d'))+ '/' + now.strftime('%Y%m%d_%H%M') + '.mp4'
csvname = './csv/{}'.format(now.strftime('%Y_%m%d')) + '/' + now.strftime('%Y%m%d_%H%M') + '.csv'
...
if __name__ == '__main__':
    ...
    writer.release()
    logger.debug('to_csv+')
    dfs.to_csv(csvname, encoding="utf-8")
    logger.debug('output+')
    out_rad(now,now,csvname)
